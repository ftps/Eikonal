...
uint writeThings(const std::string& filename, const FastMarch* const f, const pairXY& res)
{
	std::fstream fp;
	uint i = 0;
...
void printHelp()
{
	...
	std::cout << ".\\eikonal.exe FILE.NAME (-s XXX) (-f XXX) (-p) (-m) (-r XXX) (-rf XXX)\n" << std::endl;
	...
	std::cout << "-e: export mesh file (only if mesh is generated by the program)" << std::endl;
	std::cout << "-r: mesh refinement level from 0 (extra coarse) to 4 (extra fine)" << std::endl;
	std::cout << "-rf: give the mesh size factor directly instead of the pre-defined levels\n" << std::endl;
	std::cout << "If both -r and -rf are given, the latter one will be chosen.\n" << std::endl;
	std::cout << "Defaults:\nSample surfaces: 25\nRefinement level: 0.05" << std::endl;
	std::cout << "Default refinement levels (-r): 0.08, 0.06, 0.05, 0.04, 0.035 (0 to 4 respectively)" << std::endl;
	//std::cout << "\nExample:" << std::endl;
	...
	*/
	...
	for(int i = 2; i < argc; ++i){
		...
		// flame spreading speed
			...
			//std::cout << "\nFlame spreading speed is " << f_speed << "." << std::endl; 
		}
	...
	if(force_mesh) f = new FastMarch(minfo, b_in, f_speed, fN);
	else f = new FastMarch(filename + ".diff", b_in, f_speed, fN);
	...
	file_version = writeThings(filename, f, res);
	//std::cout << "Info\t: Done.\n" << std::endl;

	std::cout << ((force_mesh) ? "Info\t: Meshing time: " : "Info\t: Reading mesh time: ") << std::chrono::duration_cast<std::chrono::seconds>(mid1 - start).count() << " seconds." << std::endl;
	...
	if(prt){
		if(fs::exists(fs::path("plotBurnArea.py"))){
			aux = "python plotBurnArea.py \"" + filename + "_" + std::to_string(file_version) + ".txt\"";
			aux_i = system(aux.c_str());
		}
		else{
